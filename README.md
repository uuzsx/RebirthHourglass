...
当前分支面向 Minecraft **26.3 + NeoForge 26.3.0.16-beta + Java 25** 的重新实现。模组 ID 为 `rebirth_hourglass`。需要在客户端和服务端同时安装。
...
主模组 JAR 输出到 `build/libs/`。文件名含 `sources` 的 JAR 是源码，不要作为模组安装。构建固定使用 Gradle 9.2.1、ModDevGradle 2.0.147、NeoForge 26.3.0.16-beta。
...
此分支固定使用 NeoForge 26.3.0.16-beta（beta 构建）。

26.3 分支包含死亡掉落流程的 Mixin 兼容处理，使新版背包、护甲和副手掉落进入同一个保护流程。

兼容性与范围：

- 严格针对 Minecraft 26.3；请使用对应分支的 JAR，不将它当成其他 Minecraft 版本的通用包。
